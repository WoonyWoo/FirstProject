...
function renderMinos(moveType){
    ...
    movingArray = [];
    for(var i=0; i<divArray.length; i++){
        for(var j=0; j<divArray[j].length; j++){
            if(divArray[i][j].moving){
                movingArray.push(divArray[i][j]); 
            }
        }
    }
    
    movingArray.forEach(mino=>{
        mino.style.background = "white";
        mino.moving = false;
    })

    minos[type][direction].forEach(mino => { //블럭의 direction에 따른 좌표 값
